...
use super::{DisplayError, DisplayTarget};
use crate::BmpImage;
...
impl EmbeddedDisplay {
    ...
    pub fn draw_pixel(&mut self, x: i32, y: i32, nibble: u8) {
...
impl DisplayTarget for EmbeddedDisplay {
    fn width(&self) -> i32 {
        EmbeddedDisplay::width(self)
    }

    fn height(&self) -> i32 {
        EmbeddedDisplay::height(self)
    }

    fn clear(&mut self, color: Gray4) {
        EmbeddedDisplay::clear(self, color);
    }

    fn flush(&mut self) -> Result<(), DisplayError> {
        EmbeddedDisplay::flush(self).map_err(DisplayError::Embedded)
    }

    fn draw_pixel(&mut self, x: i32, y: i32, nibble: u8) {
        EmbeddedDisplay::draw_pixel(self, x, y, nibble);
    }

    fn draw_bitmap(&mut self, image: &BmpImage, x: i32, y: i32) {
        EmbeddedDisplay::draw_bitmap(self, image, x, y);
    }
}
